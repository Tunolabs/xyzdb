...
**State:** current as of `1.1` (2026-08-01). The core engine architecture below is accurate; sections describing pre-v0.6 surfaces remain valid where the engine is unchanged — see the pre-1.0 changelog in `docs/releases/CHANGELOG-pre-1.0.md` for per-version deltas. Notable changes since v0.5.0: `gravity_hash` widened 21 → 48 bits with `MANIFEST_VERSION` 4 (v0.6.0-pre); high-cardinality aggregate ghosts spill their group rollups to disk (**lightweight ghosts**, §4.3, v0.7.6); the **GravitySpec keel + D1** value-only canonical placement hash and cacheable per-SST metadata landed in v0.8.0. **The engine is a single-tier hardened LSM.** v0.8.8 added the **vector subsystem** (§3.7): a 5th `vectors` keyspace and a fused, gravity-bounded EXACT `NEAREST`. **1.0 widened `SpatialKey` to 24 bytes** with a `sat` satellite axis and `MANIFEST_VERSION` 5 (§2.3); **the axis is now live** — `SATELLITE BY <field> IN "lobe"` sub-divides one gravity bucket so a query pinning both fields reads only the matching rows (§2.3.1). Opt-in per lobe: a lobe that does not declare it keeps every record at satellite 0 and behaves exactly as before: a pre-1.0 (0.8.x, 22-byte / v4) data dir is rejected on open — there is no in-place migration, the upgrade path is re-ingestion from source.
...
**Fused `Scan`+`Nearest` (`xyzdb-engine/src/ops/nearest/mod.rs`, `execute_scan_nearest`).** For a `NEAREST` over a gravity bucket, the fast path ranges the `vectors` column (V5) / hoisted prefix (V3/V4) over the bucket key range, scores each candidate **zero-copy** (no full-record materialisation per candidate), keeps a bounded top-k min-heap, and fully hydrates only the surviving top-k. A flat-`AND` residual on a non-gravity field does not disqualify it: since M2.3 the path hydrates in score order and applies the residual until k records **pass** (hydrate-until-k), which is exact and deserialises only as many records as it takes to find the k winners; `OR`/`NOT` residuals still fall back. **The plan is chosen whenever the pipeline STARTS with `SCAN | NEAREST`**, and any further steps run over its result — it is not gated on the pipeline being exactly those two steps, because a projection appended at the end would then have selected a smaller candidate set (`tests/pipeline_fusion.rs`). The result is **bit-identical** to the unfused reference path (`scan → execute_nearest`), checked by `tests/scan_nearest_fused.rs`. The pruned-cosine core (`xyzdb-core/src/distance.rs`, `cosine_pruned`) uses a Cauchy–Schwarz upper bound (`dot_partial + ‖a_tail‖·‖b_tail‖`) to abort a candidate that provably cannot reach the current k-th best; survivors score bit-identically to the unpruned `similarity_indexed`.
...
- `GHOST_META_FORMAT` (xyzdb-engine ghost persistence): **0x09** (current, [`crates/engine/src/ghost/mod.rs`](../crates/engine/src/ghost/mod.rs)). One format byte at the head of every persisted ghost-meta record. Unlike the two above, a mismatch is not fatal: the record is **skipped** with a warning and the ghost rebuilds, since a stale meta record is recoverable state, not data. Per-release bumps are in the changelogs.
...
- `invariant_guards` (1.1) — counters for states the read path assumes impossible: the L1+ non-overlap guard (with a per-keyspace breakdown) and duplicate-anchor inserts prevented after recovery. **Correctness signals, not capacity metrics** — a non-zero value is an engine bug, not a load condition.
- `recovered_from_wal` (1.1) — whether this process replayed WAL at open, and therefore re-confirms anchor misses without the bloom for its whole life. A degraded *mode*, not a fault: correct, but it costs a level descent per anchor miss until restart, published so slower anchored writes can be explained rather than guessed.

The last two exist because a guard that `debug_assert!`s and otherwise logs is **invisible in release to any embedder that links the crate without a `tracing` subscriber** — which is most of them. An invariant the engine owns cannot delegate its visibility to the caller's logging setup, so it became state the engine reports. This is Principle 2 of §11 applied to a runtime guard rather than to a comment. Every known keyspace is emitted even at zero: a missing series is indistinguishable from a scrape gap, and *"we did not look"* must not read like *"it did not happen"*.
