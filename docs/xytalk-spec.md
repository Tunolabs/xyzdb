...
| `strategy` | Which order the candidates were walked in. Two values are defined: `"score_order"` (best-first ⇒ the rows are a true **prefix** of the answer) and `"key_order"` (sequential I/O ⇒ the rows are the best of a contiguous **key region**, and the unwalked part may hold better ones). **1.1.0 emits only `"score_order"`** — no shipped code path selects the other traversal. It names the fact, not the implementation. |
