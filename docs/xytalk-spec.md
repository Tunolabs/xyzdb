...
**State:** current as of `1.1.1` (2026-08-06). 1.1.1 changed no grammar and no
response shape; it corrected three things this document asserted and the engine
denies — `LOBE` and `ANCHOR` are **not** idempotent (§2.2), an upsert **merges**
rather than replaces (§2.4), and a link written by the standalone `LINK`
statement is **not** visible to `PULL` (§2.9). 1.1 adds one statement, `SATELLITE BY <field> IN "lobe"` (§2.2.2, the sub-gravity axis), and one response field, `budget_stop` on a truncated `NEAREST` (§2.20) — both additive; every 1.0 statement parses unchanged. The rest of this document was refreshed §-by-§ against the parser for the **xyTalk v1** alignment. One form per concept, with the previous spelling kept as a live alias where the change was cosmetic. The v1 surface changes, all reflected below: `TAKE` is the canonical top-N / truncate step (`TOP` is a deprecated alias); `NEAREST` is canonical with a phrase form (`NEAREST k BY field TO q [USING metric]`) and the function call as an alias — **`ORBIT` is removed**; `PURGE "lobe"` is the explicit total-delete verb and `DELETE` now requires a `WHERE`; `SET` / `DELETE` / `LINK` / `SCAN GHOST` accept the full `OR`/`NOT`/`IN` WHERE tree; `CREATE GHOST` accepts the query pipeline form; `IN [...]` is canonical (`IN (...)` an alias); `count(*)` is accepted as an alias of `count()`; `FIND` rejects `OR`/`NOT` at parse with an error that points to `SCAN`; `SCAN ORDER BY` requires `LIMIT`. The parser is the source of truth in every conflict. The engine is single-tier.
...
- **Max 10,000 records per batch.** A larger batch is rejected whole with an
  ...
  not across chunks).
- **`ON CONFLICT UPDATE` updates**, exactly as the single-`PUT` form does (§2.3): a record whose anchor value already exists is merged into the existing record instead of inserted. Two records inside one batch that share an anchor value collapse the same way — the first is inserted, the second merges onto it. Without the clause, a collision is still `DuplicateAnchor` and the whole batch is rejected.
- **A batch upsert updates in place and does not re-bucket** — the same declared consequence the single-statement form carries. If the merge changes the **gravity** field or the **satellite** axis, the record stays in its old bucket and a bounded query on the new value will not find it (§2.2.2). Use `SET` to move a record; the batch door does not change that.
- **Atomicity with `ON CONFLICT UPDATE` is per half.** The inserts commit as one atomic batch; the updates are applied after it, one commit each. A mixed batch is therefore not all-or-nothing across both halves. Without the clause, the batch is atomic as described above.
- The reported count is the number of records the batch **wrote** — inserts plus updates. `first_lid` / `last_lid` name the **inserted** records only; a batch in which every record turned out to be an update reports them as the zero LID. Read the count, not the LIDs, to confirm a batch landed.
...
> **`AUTOLINK` does not exist.** There is no `AUTOLINK`, `SHOW AUTOLINK` or `AUTOLINK APPLY` — not in the parser and not anywhere in the engine. `AUTOANCHOR APPLY` (§2.17) is a different statement and is real. The two spellings fail differently, which is worth knowing if you are reading an error: `AUTOLINK APPLY …` gives `Unknown command: 'AUTOLINK'`, while `SHOW AUTOLINK` gives a raw parser error (`Parsing Error: Error { input: "AUTOLINK", code: Tag }`) because `SHOW` itself parses and only its argument fails — see `KNOWN-ISSUES.md` on unreadable parse errors. The one place the name survives is the operational suite [`tools/validation/src/suites/s08_autodiscovery.rs`](../tools/validation/src/suites/s08_autodiscovery.rs), whose AUTOLINK cases cannot pass against any current build. Nothing is planned; the statements were never implemented, not withdrawn.
...
| `strategy` | Which order the candidates were walked in. Two values are defined: `"score_order"` (best-first ⇒ the rows are a true **prefix** of the answer) and `"key_order"` (sequential I/O ⇒ the rows are the best of a contiguous **key region**, and the unwalked part may hold better ones). **No shipped release emits `"key_order"`** — no code path selects the other traversal. It names the fact, not the implementation. |
...
> **The `"key_order"` column describes a value no shipped release sends.** It is specified because the field's *contract* is the pair, and a client that switches on it today keeps working when the second traversal ships — not because the engine takes that path. Do not write a client that expects to see `"key_order"`, and do not read this table as evidence that key-order truncation happens. A future minor will say when it starts.
