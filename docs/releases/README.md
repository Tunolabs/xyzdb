...
|---|---|---|
| [v1.1.0](v1.1.0.md) | 2026-08-03 | **Sub-gravity (`SATELLITE BY`)** — a third foundational axis that bounds `SCAN`/`AGGREGATE`/`NEAREST` to one sub-bucket. **`budget_stop`** says what a truncated `NEAREST` partial actually is, including which traversal produced it. **Three correctness fixes that could each return a wrong answer silently**: a `UNIQUE` anchor duplicated after an unclean restart, and `ON CONFLICT UPDATE` reaching neither aggregate ghosts (stale sums served as current) nor the record cache (read-your-own-write). **Invariant guards became `STATS` + `/metrics` state.** Additive and backward compatible — a 1.0.x data directory opens unchanged; **run `REFRESH GHOST` once** if you have aggregate ghosts over lobes that take upserts. |
