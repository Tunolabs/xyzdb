...
impl XyzdbServer {
    ...
    #[tool(
        description = "Snapshot of xyzDB engine internals: keyspace stats (memtables, SSTables, compaction counters), block cache, ghosts, sync thread health, process memory, cgroup memory. Same JSON shape as the /stats endpoint on xyzdb-server's TCP port. Prefer this tool over `query` with `SHOW STATS` — the response is structured JSON, not human text. Two fields are correctness signals rather than capacity numbers, and neither is a tuning knob. `invariant_guards` is an object of counters for internal consistency checks that tripped; they never reset. A non-zero `level_overlap` (also broken down by keyspace) means point reads can miss keys a scan still finds — report it. A non-zero `anchor_bloom_false_negative` means the guard did its job and stopped a UNIQUE key from being duplicated after an unclean restart, so no data was harmed, but the deployment is meeting a known defect and the operator should be told. `recovered_from_wal` is true when the last start replayed the write-ahead log, i.e. the previous shutdown was unclean. Everything else is capacity and can be read as load."
    )]
...
- SATELLITE BY field IN "lobe" — declare a sub-gravity axis: it splits each gravity bucket into sub-ranges, so a query pinning BOTH the gravity field and this one reads only the matching sub-range. It only means something alongside a declared gravity, and it rewards equality (`field = X` reads one sub-range) over a range (`field < X` sweeps the whole bucket). Same rows, same order either way — purely which query is cheap. Read it back with describe_lobe.
...
Partial results — read this before treating a response as complete. Two different fields say "there is more", and neither may be dropped:
- `has_more` with a `cursor` — an ordinary next page. Fetch it with the cursor.
- `budget_stop` — only on NEAREST, and it means the server's latency airbag cut the scoring pass. You got the highest-scoring records it managed to score (prefix-correct, not a random sample), with `has_more: true` and `cursor: null` — there is no page to resume, because resuming would repeat the whole pass. It carries `examined` / `candidates` / `found` and `strategy` (which traversal produced the partial). Treating this as the complete top-k is the mistake to avoid: say the answer is partial, or narrow the scope (pin the gravity field, add a satellite equality, tighten WHERE) and re-run. A pipeline that appends SET, DELETE or AGGREGATE after a truncated NEAREST is refused with an error rather than silently writing or totalling over the partial set; read-only steps such as SHAPE compose fine and the flag travels with them.
...
Tool selection: prefer the dedicated `list_lobes`, `describe_lobe`, and `stats` tools for schema discovery and engine introspection — they parse the `SHOW` output into structured JSON. Use `query` for actual data operations (PUT/FIND/SCAN/NEAREST/AGGREGATE/TAKE/SET/DELETE/PURGE/LINK and VECTOR/GRAVITY/CREATE GHOST declarations) and for SHOW commands not covered by a dedicated tool."#
    ...
    /// pre-flight check.
    #[tool(
        description = "Full schema for a single lobe: anchors, ghosts (those whose source_lobe matches), profile (pinned fields, learned scan patterns, active ghost count), and the searchable vector field. Returns a structured object with `anchors`, `ghosts`, `profile` (each independently fallible — on failure replaced by `{\"error\": \"...\"}`) and top-level `gravity`, `vector` and `satellite`: `gravity` names the lobe's PRIMARY axis and is `null` if none is declared — a query pinning it reads one bucket while one that does not sweeps the lobe, so read this FIRST; `vector` is `null` if the lobe has no searchable field, else `{\"field\": ..., \"dim\": ...}`; `satellite` is `null` unless the lobe declares a sub-gravity axis, in which case it names that field (a sub-range OF the gravity bucket, so it only means something together with `gravity`). The satellite axis changes WHICH QUERY IS CHEAP: an equality on it (`field = X`) reads one sub-range of the gravity bucket, while a range (`field < X`) sweeps the whole parent — so prefer equality on the satellite field when you can. A `null` `dim` means the dimension is not fixed yet (declared but no embedding written) — you may choose it on the first write; a set `dim` means every `NEAREST` query vector must match it (the engine never embeds; you supply the vector). If the lobe does not exist, the tool returns INVALID_PARAMS with no partial body. Prefer this over `query` with `SHOW ANCHORS` / `SHOW GHOSTS` / `SHOW PROFILE` separately — it composes the three SHOW calls and parses the result. Use after `list_lobes` to plan queries."
    )]
