...
impl Engine {
    ...
    pub(super) fn load_satellite_fields(
        ...
        let Ok(entries) = dictionary.prefix_iter(&crate::reserved_keys::SATELLITE) else {
            return result;
        };
        for entry in entries {
            let Some(id_bytes) = entry.key.get(crate::reserved_keys::SATELLITE.len()..) else {
                continue;
            };
            ...
            if let Some(spec) = crate::satellite_spec::SatelliteSpec::decode(&entry.value) {
