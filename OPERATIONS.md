...
> The three `xyzdb_invariant_*` series are **correctness signals, not capacity metrics**. They count states the read path assumes impossible, so any non-zero value is a bug in the engine rather than a load condition — page, do not tune. They are emitted for every keyspace even at zero: a missing series is indistinguishable from a scrape gap, and "we did not look" must not read like "it did not happen". `xyzdb_recovered_from_wal` is different in kind: it reports a degraded MODE (an anchor miss is re-confirmed without the bloom for the life of a process that replayed WAL), which is correct but costs a level descent per anchor miss until restart — it is published so slower writes can be explained instead of guessed.
