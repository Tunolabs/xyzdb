...
| `GHOST_META_FORMAT` (ghost metadata record header) | `0x09` | [`crates/engine/src/ghost/mod.rs:300`](crates/engine/src/ghost/mod.rs#L300) |

Relative to the 0.8.x line, the 0.9.x on-disk format widened `SpatialKey` from 22 to 24 bytes — adding a 2-byte satellite (sub-gravity) axis (`crates/core/src/key.rs`) — and bumped `MANIFEST_VERSION` from 4 to 5. The axis was reserved and always `0` until 1.1 made it live: a lobe that declares `SATELLITE BY <field>` stores `hash16(field)` there, and a lobe that does not keeps every record at satellite 0, where the physical ordering is byte-for-byte the pre-1.1 one. Declaring the axis needed no format change, which is why 1.1 is an in-place upgrade (§8) and this table is unchanged. The byte offsets shifted at 0.9.0, so data written by a v4 / 22-byte build is rejected on open (`Error::IncompatibleFormat`); there is no in-place widening. Within a patch series both constants stay at the values above; a release that bumps either one ships with explicit upgrade notes rather than a silent format change.
